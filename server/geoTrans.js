...
GeoTrans.prototype.geo2tm = function(in_pt, out_pt) {
  ...
  if (this.m_Ind != 0) {
    ...
    x =
      0.5 *
      this.m_arMajor *
      this.m_arScaleFactor[this.dsttype] *
    ...
    if (in_pt.y < 0) {
      ...
      y =
        this.m_arMajor *
        this.m_arScaleFactor[this.dsttype] *
        (con - this.m_arLatCenter[this.dsttype]);
    }
  }
      ...
      (1.0 +
        (als / 6.0) *
          (1.0 -
            ...
            c +
            (als / 20.0) *
              (5.0 - 18.0 * t + t * t + 72.0 * c - 58.0 * this.m_Esp))) +
    this.m_arFalseEasting[this.dsttype];
  out_pt.y =
    this.m_arScaleFactor[this.dsttype] *
      (ml -
        this.dst_m +
                ...
                (5.0 -
                  ...
                  (als / 30.0) *
                    (61.0 -
                      ...
                      600.0 * c -
                      330.0 * this.m_Esp))))) +
    this.m_arFalseNorthing[this.dsttype];
};
...
GeoTrans.prototype.tm2geo = function(in_pt, out_pt) {
  ...
  if (this.m_Ind != 0) {
    var f = Math.exp(
      in_pt.x / (this.m_arMajor * this.m_arScaleFactor[this.srctype])
    );
    var g = 0.5 * (f - 1.0 / f);
    var temp =
      this.m_arLatCenter[this.srctype] +
      in_pt.y / (this.m_arMajor * this.m_arScaleFactor[this.srctype]);
    var h = Math.cos(temp);
  ...
  var con =
    (this.src_m + in_pt.y / this.m_arScaleFactor[this.srctype]) /
    this.m_arMajor;
  var phi = con;
  ...
  while (true) {
    var delta_Phi =
      (con +
        this.e1fn(this.m_Es) * Math.sin(2.0 * phi) -
        this.e2fn(this.m_Es) * Math.sin(4.0 * phi) +
        this.e3fn(this.m_Es) * Math.sin(6.0 * phi)) /
        this.e0fn(this.m_Es) -
      phi;
    phi = phi + delta_Phi;
            ...
            (5.0 +
              ...
              (ds / 30.0) *
                (61.0 +
                  ...
                  3.0 * cs)));
    out_pt.x =
      this.m_arLonCenter[this.srctype] +
      (d *
        (1.0 -
          (ds / 6.0) *
            (1.0 +
...
//
const katecToLatLng = (mapx, mapy) => {
  let geo = new GeoTrans();
  geo.init("katec", "geo");
...
// console.log("경도 : " + out_pt.x + "\n위도 : " + out_pt.y);

//역변환
// geo.init("geo", "katec");
// out_pt = geo.conv(out_pt);

// console.log("[KATEC 좌표]\nx : " + out_pt.x + "\ny : " + out_pt.y);
